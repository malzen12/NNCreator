...
auto ParamWidget::getParam() -> NNParam& { return m_Param; }

auto ParamWidget::collectValue() -> const NNParam& {
  if (m_Param.isEnum()) {
    m_Param.setValue(static_cast<QComboBox*>(m_pEditorWidget)->currentIndex(), true);
  } else {
...
void ParamWidget::initGuiEnum() {
  ...
  pWidget->setCurrentIndex(static_cast<int>(m_Param.getValue().toUInt()));
