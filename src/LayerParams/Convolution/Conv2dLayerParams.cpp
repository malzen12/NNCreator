...
///< @todo to aux
inline std::size_t calc_output_axis(std::size_t sInput, std::size_t sKernel, std::size_t sStride,
                                    std::size_t sPadding, std::size_t sDilitation) {
...
std::vector<std::size_t> Conv2dLayerParams::calcOutputSize(const InputSizeType &vInputSizes) const {
  ...
  vRes.back()           = calc_output_axis(vInputSize.back(), lKernel[1].toUInt(), lStride[1].toUInt(),
                                           lPadding[1].toUInt(), lDilitation[1].toUInt());
  vRes[vRes.size() - 2] = calc_output_axis(vInputSize[vInputSize.size() - 2], lKernel[0].toUInt(),
                                           lStride[0].toUInt(), lPadding[0].toUInt(),
                                           lDilitation[0].toUInt());
